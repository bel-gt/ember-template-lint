...
module.exports = class NoBareStrings extends Rule {
  ...
  _addStringToTranslations(string, locale = 'en') {
    ...
    const lastKeyPathSegment = normalize(camelCase(string)).slice(0, 25);
    const keyPathSegments = this._filePath.split(path.sep).map(camelCase).map(normalize);
    keyPathSegments.pop(); // drop the app
    keyPathSegments.shift(); // drop the filename
    const keyPath = [...keyPathSegments, lastKeyPathSegment].join('.');
    ...
    if (fs.existsSync(filePath)) {
      try {
        ...
        parent[lastKeyPathSegment] = string;
